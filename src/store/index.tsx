...
import type { AppState, AppAction, TopicProgress, RecallResult } from '@/types';
...
function getNextReviewDate(result: RecallResult): string {
  ...
  // "missed" = same day (0 days offset keeps same date)
...
function reducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'COMPLETE_SESSION': {
      ...
      const session = { date, topicIds, completed: true, minutesSpent };
      return {
        ...state,
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AppContext.Provider value={{ state, dispatch, updateTopicProgress, toggleFavorite, isFavorite }}>
      {children}
    </AppContext.Provider>
  );
}
